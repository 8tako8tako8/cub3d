...
/*   Updated: 2020/12/28 12:40:22 by kmorimot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parse_line_e(t_all *all, char **line)
{
	int		i;
...
/* 	if ((*line)[0] == 'R' && (*line)[1] == ' ')
	...
	if (all->flag.eflag == 0 && ft_ismap(all, *line))
		...
		all->map.end = 1;
	else if (all->flag.eflag == 0 && all->map.start == 1 && all->map.end == 1 && ft_check_last(*line) == -1)
