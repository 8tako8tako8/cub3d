...
/*   Updated: 2020/12/22 02:26:47 by kmorimot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_calc_wall_tex(t_all *all, int *x)
{
	...
	all->tex.pos = (all->ray.drawstart - all->win_r.y / 2 + all->ray.lineheight / 2) 
																	* all->tex.step;
	y = all->ray.drawstart;
	while (y < all->ray.drawend)
	{
		...
		all->tex.color = all->mlx.texture[all->tex.num][(all->img.tex_height[all->tex.num]) 
