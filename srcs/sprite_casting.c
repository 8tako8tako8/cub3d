...
/*   Updated: 2020/12/22 01:51:53 by kmorimot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_calc_sprite_tex(t_all *all)
{
	...
	while (x < all->spr.draw_end_x)
	{
		all->spr.tex_x = (int)((256 * (x - (-all->spr.spr_width / 2 
			+ all->spr.spr_screen_x)) * all->img.tex_width[4] / all->spr.spr_width) / 256);
									...
									&& all->spr.transform_y < all->spr.zbuffer[x])
		{
			...
			{
				all->spr.d = (y-all->spr.move_screen) * 256 - all->win_r.y * 128 
																+ all->spr.spr_height * 128;
				all->spr.tex_y = ((all->spr.d * all->img.tex_height[4]) / all->spr.spr_height) / 256;
				all->spr.color = all->mlx.texture[4][all->img.tex_width[4] * all->spr.tex_y 
																			+ all->spr.tex_x];
...
void	ft_set_elements_for_sprite_drawing(t_all *all, int *i)
{
	...
	all->spr.transform_x = all->spr.invdet * (all->player.dir_y * all->spr.relative_x 
								- all->player.dir_x * all->spr.relative_y);
	all->spr.transform_y = all->spr.invdet * (-all->player.plane_y * all->spr.relative_x 
